...
func TestMultiByteReader_Len(t *testing.T) {
	{
		r := NewMultiByteReader()

		var expectedLen int = 0
		if r.Len() != expectedLen {
			t.Errorf("assert 'MultiByteReader.Len() offset':: expected '%+v', got '%+v'", expectedLen, r.Len())
		}
	}
...
func TestMultiByteReader_Size(t *testing.T) {
...

	{
		...
		if r.Size() != expectedSize {
			t.Errorf("assert 'MultiByteReader.Size() offset':: expected '%+v', got '%+v'", expectedSize, r.Size())
		}
	}
...
func TestMultiByteReader_ReadByte(t *testing.T) {
	{
		...
		expectetErr := io.EOF
		if err != expectetErr {
			t.Errorf("assert 'MultiByteReader.ReadByte() error':: expected '%+v', got '%+v'", expectetErr, err)
		}
		expectetByte := byte(0)
		if b != expectetByte {
			t.Errorf("assert 'MultiByteReader.ReadByte() byte':: expected '%+v', got '%+v'", expectetByte, b)
		}
	}
...
func TestMultiByteReader_Read(t *testing.T) {
	...
	expectetN := 9
	if n != expectetN {
		t.Errorf("assert 'MultiByteReader.ReadByte() byte':: expected '%+v', got '%+v'", expectetN, n)
	}

	expectetBuf := []byte("foobarbaz")
	if !reflect.DeepEqual(buf, expectetBuf) {
		t.Errorf("assert 'MultiByteReader.ReadByte() byte':: expected '%+v', got '%+v'", expectetBuf, buf)
	}
}

func TestMultiByteReader_Read_WithNested(t *testing.T) {
	r1 := bytes.NewReader([]byte("foo"))
	r2 := NewMultiByteReader(
...
func TestMultiByteReader_Seek(t *testing.T) {
...

	{
		...
		if offset != expectetOffset {
			t.Errorf("assert 'MultiByteReader.Seek() offset':: expected '%+v', got '%+v'", expectetOffset, offset)
		}
	}
